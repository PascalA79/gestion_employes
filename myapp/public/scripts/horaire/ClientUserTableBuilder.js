BuildUserTableData(roles, quarts, debut, fin){
...
function FillRow(rowContent){
    const row = $("tr");
    for(cell of rowContent){
        row.append(FillCell(cell.content, cell.attributes));
    }
    return row;
}

function FillCell(cellContent, cellAttributes){
    const cell = $("td");
...
function FillCell(startTime, quarts, roles){
    const quart = quarts.
}
