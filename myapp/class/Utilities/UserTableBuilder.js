...
module.exports.BuildUserTableData = function BuildUserTableData(roles, planchers, quarts, debut, fin){
    const days = DateUtilities.getDaysDate(debut, fin);
    const hours = DateUtilities.getHours({});
    const data = hours.map(h => BuildHourRow(h, days, quarts, roles, planchers));
...
function BuildHourRow(hour, days, quarts, roles, planchers){
    const row = [];
    for(day of days){
        const cell = BuildCell(hour, day, quarts, roles, planchers);
...
function BuildCell(hour, day, quarts, roles, planchers){
    ...
    DateUtilities.isMidnightDate(dateTime)) return BuildShiftCell(currentShift, dateTime, roles, planchers);
}
...
function BuildShiftCell(quart, dateTime, roles, planchers){
    ...
    td.content = `${role.nom} (${plancher.nom}): ${strDebut} - ${strFin}`;
